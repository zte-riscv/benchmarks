...
# 默认参数
BENCHMARK_DIR="${1:-go_benchmarks_arm64}"
RESULTS_DIR="${2:-results-arm64}"
BENCHNUM="${3:-10}"
BENCHTIME="${4:-5s}"
...
if [ ! -d "$BENCHMARK_DIR" ]; then
    ...
    echo "  benchmark_dir: Directory containing benchmark binaries (default: go_benchmarks_arm64)"
    echo "  results_dir:   Directory to save results (default: results-arm64)"
...
# 运行 JSON 基准测试
if [ -f "$BENCHMARK_DIR/json-arm64" ]; then
    echo "Running JSON benchmark (benchnum=$BENCHNUM, benchtime=$BENCHTIME)..."
    if "$BENCHMARK_DIR/json-arm64" -benchnum "$BENCHNUM" -benchtime "$BENCHTIME" 2>&1 | tee "$RESULTS_DIR/json-results.txt"; then
    ...
    echo "⚠ JSON benchmark binary not found: $BENCHMARK_DIR/json-arm64"
fi

# 运行 Garbage 基准测试
if [ -f "$BENCHMARK_DIR/garbage-arm64" ]; then
    echo "Running Garbage benchmark (benchnum=$BENCHNUM, benchtime=$BENCHTIME)..."
    if "$BENCHMARK_DIR/garbage-arm64" -benchnum "$BENCHNUM" -benchtime "$BENCHTIME" 2>&1 | tee "$RESULTS_DIR/garbage-results.txt"; then
    ...
    echo "⚠ Garbage benchmark binary not found: $BENCHMARK_DIR/garbage-arm64"
fi

# 运行 GC Latency 基准测试
# 注意：gc_latency 不使用 driver 包，不支持 -benchnum 和 -benchtime 参数
# 它只支持自己的参数：-how (stack/heap/global), -fluff, -trace
if [ -f "$BENCHMARK_DIR/gc_latency-arm64" ]; then
    echo "Running GC Latency benchmark (note: does not support benchnum/benchtime)..."
    if "$BENCHMARK_DIR/gc_latency-arm64" 2>&1 | tee "$RESULTS_DIR/gc_latency-results.txt"; then
    ...
    echo "⚠ GC Latency benchmark binary not found: $BENCHMARK_DIR/gc_latency-arm64"
fi
